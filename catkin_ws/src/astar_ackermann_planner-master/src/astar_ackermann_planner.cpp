...
namespace astar_ackermann_planner {
    ...
    void AStarAckermannPlanner::loadParameters() {
        ...
        nh.param<double>(std::string("turning_radius"), turning_radius_, 13.0);
        nh.param<double>("step_size", step_size_, 0.2);
    ...
    PoseWithDist AStarAckermannPlanner::turnLeft(const Pose &pos, double dth) const {
    ...
    PoseWithDist AStarAckermannPlanner::goStraight(const Pose &pos) const {
    ...
    PoseWithDist AStarAckermannPlanner::turnRight(const Pose &pos, double dth) const {
        double pos_dx = sin(pos.th) * turning_radius_;
        double pos_dy = -cos(pos.th) * turning_radius_;
        double c_x = pos.x - pos_dx;
        double c_y = pos.y - pos_dy;
    ...
    vector<PoseWithDist> AStarAckermannPlanner::getNeighbors(const Pose &pos) const {
        double dth = step_size_ / turning_radius_;
    ...
    double AStarAckermannPlanner::distEstimate(const Pose &pose1, const Pose &pose2) const {
...
//      // Create a Costmap2DROS instance
//     tf2_ros::Buffer tfBuffer;
//     costmap_2d::Costmap2DROS costmap("costmap", tfBuffer);
//     // Access the costmap instance
//     costmap_2d::Costmap2D* costmap_ptr = costmap.getCostmap();
//     planner.initialize("Planner", new astar_ackermann_planner::CostmapAdapter(costmap_ptr));
//     std::vector<geometry_msgs::PoseStamped> plan;
